...
public class DrawHelper1 {
    ...
    public void draw(FaceRectView faceRectView, List<DrawInfo1> drawInfo1List) {
        if (faceRectView == null) {
            return;
        }
        faceRectView.clearFaceInfo();
        if (drawInfo1List == null || drawInfo1List.size() == 0) {
            return;
        }
        for (DrawInfo1 drawInfo1 : drawInfo1List) {
            drawInfo1.setRect(adjustRect(drawInfo1.getRect(), previewWidth, previewHeight, canvasWidth, canvasHeight, cameraDisplayOrientation, cameraId,
                    isMirror, false, false));
        }
        faceRectView.addFaceInfo(drawInfo1List);
    }

    public void draw2(MyFaceRectView faceRectView, List<DrawInfo1> drawInfo1List) {
        if (faceRectView == null) {
            return;
        }
        faceRectView.clearFaceInfo();
    ...
    /**
     * 绘制数据信息到view上，若 {@link DrawInfo1#name} 不为null则绘制 {@link DrawInfo1#name}
     *
     * @param canvas            需要被绘制的view的canvas
     * @param drawInfo1          绘制信息
     * @param color             绘制的颜色
     * @param faceRectThickness 人脸框厚度
     */
    public static void drawFaceRect(Canvas canvas, DrawInfo1 drawInfo1, int color, int faceRectThickness) {
        if (canvas == null || drawInfo1 == null) {
            return;
        }
        ...
        if (drawInfo1.getName() == null) {
            paint.setStyle(Paint.Style.FILL_AND_STROKE);
            paint.setTextSize(rect.width() / 8);

            String str = (drawInfo1.getSex() == GenderInfo.MALE ? "MALE" : (drawInfo1.getSex() == GenderInfo.FEMALE ? "FEMALE" : "UNKNOWN"))
                    + ","
                    + (drawInfo1.getAge() == AgeInfo.UNKNOWN_AGE ? "UNKNWON" : drawInfo1.getAge())
                    + ","
                    + (drawInfo1.getLiveness() == LivenessInfo.ALIVE ? "ALIVE" : (drawInfo1.getLiveness() == LivenessInfo.NOT_ALIVE ? "NOT_ALIVE" : "UNKNOWN"));
            canvas.drawText(str, rect.left, rect.top - 10, paint);
        } else {
            paint.setStyle(Paint.Style.FILL_AND_STROKE);
            paint.setTextSize(rect.width() / 8);
            canvas.drawText(drawInfo1.getName(), rect.left, rect.top - 10, paint);
        }
    }
     ...
     * @param faceRectThickness 人脸框厚度
     */
    public static void drawMyFaceRect(Canvas canvas, DrawInfo1 drawInfo1, int color, int faceRectThickness) {
        if (canvas == null || drawInfo1 == null) {
            return;
        }
        Paint paint = new Paint();
