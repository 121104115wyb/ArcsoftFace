...
import com.ytlz.arcsoftface.model.FaceRegisterInfo;
import com.ytlz.arcsoftface.util.ImageUtil;
 ...
 * 人脸库操作类，包含注册和搜索
 */
public class FaceServer1 {
    private static final String TAG = "FaceServer1";
    public static final String IMG_SUFFIX = ".jpg";
    private static FaceEngine faceEngine = null;
    private static FaceServer1 faceServer1 = null;
    ...
    public static FaceServer1 getInstance() {
        if (faceServer1 == null) {
            synchronized (FaceServer1.class) {
                if (faceServer1 == null) {
                    faceServer1 = new FaceServer1();
                }
            }
        }
        return faceServer1;
    ...
    public boolean init(Context context) {
        synchronized (this) {
            if (faceEngine == null && context != null) {
                ...
                int engineCode = faceEngine.init(context, FaceEngine.ASF_DETECT_MODE_VIDEO, FaceEngine.ASF_OP_0_HIGHER_EXT, 16, 1,
                        FaceEngine.ASF_FACE_RECOGNITION | FaceEngine.ASF_FACE_DETECT | FaceEngine.ASF_AGE | FaceEngine.ASF_FACE3DANGLE | FaceEngine.ASF_GENDER);
...
//
//                        File imgFile = file.getAbsoluteFile();
...
//
//                        String imgFilePath = imgDir + File.separator + userName + IMG_SUFFIX;
//                        System.out.println("------imgFilePath"+imgFilePath);
//
//
//                        File file1 = new File(file.getAbsolutePath());
//                        long lendght = file1.length();
//
//                        long size  = lendght/1024/1024;
//
//
...
//                        System.out.println("------file.getAbsolutePath"+file1.getAbsolutePath());
     ...
     * @return 注册的所有信息
     */
    public FaceRegisterData register(Context context, byte[] nv21, int width, int height, String name, String password) {
        synchronized (this) {
            if (faceEngine == null || context == null || nv21 == null || width % 4 != 0 || nv21.length != width * height * 3 / 2) {
                return null;
            }
            FaceRegisterData faceRegisterData = new FaceRegisterData();
            faceRegisterData.setName(name);
            faceRegisterData.setPassword(password);
            if (ROOT_PATH == null) {
                ROOT_PATH = context.getFilesDir().getAbsolutePath();
                faceRegisterData.setErrorMes("路径为空");
            }
            boolean dirExists = true;
            //特征存储的文件夹
            File featureDir = new File(ROOT_PATH + File.separator + SAVE_FEATURE_DIR);
            ...
            if (!dirExists) {
                faceRegisterData.setErrorMes("特征文件夹为空");
                return faceRegisterData;
            }
            //图片存储的文件夹
            File imgDir = new File(ROOT_PATH + File.separator + SAVE_IMG_DIR);
            if (!imgDir.exists()) {
            ...
            if (!dirExists) {
                faceRegisterData.setErrorMes("图片文件夹为空");
                return faceRegisterData;
            }
...
//            if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
//                code = faceEngine.process(nv21, width, height, FaceEngine.CP_PAF_NV21, faceInfoList, processMask);
//                if (code != ErrorInfo.MOK) {
//                    faceRegisterData.setErrorMes("活体检测函数执行失败");
//                    return faceRegisterData;
//                }
//            } else {
//                faceRegisterData.setErrorMes("人脸检测失败");
//                return faceRegisterData;
//            }
...
//                faceRegisterData.setErrorMes("活体检测函数执行有误");
//                return faceRegisterData;
//            }
//            faceRegisterData.setFaceInfo(faceInfoList.get(0));
//            if (ageInfoList.size() > 0 && genderInfoList.size() > 0 && faceLivenessInfoList.size() > 0) {
//                faceRegisterData.setAgeInfo(ageInfoList.get(0));
//                faceRegisterData.setGenderInfo(genderInfoList.get(0));
//                faceRegisterData.setLivenessInfo(faceLivenessInfoList.get(0));
//            } else {
//                faceRegisterData.setErrorMes("函数执行有误");
//                return faceRegisterData;
//            }
            ...
            if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
                ...
                try {
                    ...
                    if (code == ErrorInfo.MOK) {
                        ...
                        Rect cropRect = getBestRect(width, height, faceInfoList.get(0).getRect());
                        if (cropRect == null) {
                            faceRegisterData.setErrorMes("人脸识别有误");
                            return faceRegisterData;
                        }
                        File file = new File(imgDir + File.separator + userName + IMG_SUFFIX);
                        FileOutputStream fosImage = new FileOutputStream(file);
...
//                            bitmap.compress(Bitmap.CompressFormat.JPEG, 100, fosImage);
//                            fosImage.close();
//                            Bitmap bitmap1 = BitmapFactory.decodeFile(fosImage);

                            faceRegisterData.setBitmap(bitmap);
                        }
                        ...
                        String faceFeature1 = Base64.encodeToString(faceFeature.getFeatureData(), Base64.DEFAULT);
                        faceRegisterData.setFaceFeature(faceFeature1);
...
//                        faceRegisterInfoList.add(new FaceRegisterInfo(faceFeature.getFeatureData(), userName));
     ...
     * @return 比对结果
     */
    public CompareResult1 getTopOfFaceLib(FaceFeature faceFeature) {
        ...
        if (maxSimilarIndex != -1) {
            return new CompareResult1(faceRegisterInfoList.get(maxSimilarIndex).getName(), maxSimilar);
        }
        return null;
    }
