...
public class MyFaceRectView extends View {
    private static final String TAG = "FaceRectView";
    private CopyOnWriteArrayList<DrawInfo1> faceRectList = new CopyOnWriteArrayList<>();

    public MyFaceRectView(Context context) {
        this(context, null);
    }

    public MyFaceRectView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (faceRectList != null && faceRectList.size() > 0) {
            for (int i = 0; i < faceRectList.size(); i++) {
                DrawHelper1.drawMyFaceRect(canvas, faceRectList.get(i), Color.YELLOW, 5);
            }
        }
    }
